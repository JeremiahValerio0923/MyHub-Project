...
def change_post_visibility(id_: int, db:Session = Depends(database.get_db),
    ...
    updated_post = schemas.PostCreate(title = fetched_post.content,
...
def delete_post(id_: int, post: schemas.PostCreate, db: Session = Depends(database.get_db),
                verified_user: models.User = Depends(oauth2.verify_current_user)) -> Response:

    post_to_delete = db.query(models.Post).filter_by(id=id_)
